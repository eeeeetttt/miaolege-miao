...
export default function MonthlyChallenge() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`/api/match/monthly?userId=${userId}`);
      const data = await res.json();
  ...
  const handleEnroll = async () => {
    if (!confirm('确认报名月度总决赛？需要扣除10万银两。')) {
      return;
    }

    setEnrolling(true);
    try {
      const res = await fetch('/api/match/monthly', {
        method: 'POST',
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-amber-400 animate-spin text-2xl">◌</div>
      </div>
    );
  }
  ...
  if (status === 'unauthenticated') {
    return (
      <div className="text-center py-12">
        ...
        <a href="/login" className="text-amber-400 hover:underline">前往登录</a>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-amber-900/50 to-orange-900/50 rounded-xl p-4 border border-amber-500/30">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="text-lg font-bold text-amber-300">月度总决赛</h3>
        </div>
        <p className="text-sm text-gray-300">
          月底举办为期3天的收益率大赛。上月天梯赛前100名可参加，争夺月度荣耀！
        </p>
        <div className="mt-3 grid grid-cols-3 gap-2 text-sm">
          <div className="bg-black/30 rounded-lg p-2 text-center">
            <div className="text-amber-400">免费</div>
            <div className="text-gray-400">报名费</div>
          </div>
          <div className="bg-black/30 rounded-lg p-2 text-center">
            <div className="text-amber-400">{config?.initialCapitalSilver ? (config.initialCapitalSilver / 10000).toFixed(0) : 10}万</div>
            <div className="text-gray-400">参赛本金(银两)</div>
          </div>
          <div className="bg-black/30 rounded-lg p-2 text-center">
            <div className="text-amber-400">{remainingDays}</div>
      ...
      {status === 'open' && (
        <button
          ...
          disabled={enrolling}
          className="w-full py-3 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 disabled:from-gray-600 disabled:to-gray-600 rounded-xl font-bold text-white transition"
        >
          {enrolling ? '报名中...' : `立即报名 (需${(config?.initialCapitalSilver || 100000).toLocaleString()}银两)`}
        </button>
      )}
          ...
          {rewards.map((reward: any, index: number) => (
            <div key={index} className="flex items-center justify-between text-sm">
              <span className={
                ...
                {index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `${reward.rank}`} {reward.rank}
              </span>
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {leaderboard.map((entry) => (
              <div key={entry.rank} className="flex items-center gap-3 text-sm bg-black/30 rounded-lg p-2">
                ...
                <span className="text-amber-400">{(entry.currentValue / 10000).toFixed(0)}万</span>
